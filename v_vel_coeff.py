...
def v_vel_coeff(u_vel, v_vel, dx, dy, rho, gamma, ivmax, jvmax):
    ...
    Fw[0][:] = rho * (((u_vel[0][:-2] + u_vel[1][:-2]) / 2) * dy/2 + ((u_vel[1][:-2] + u_vel[2][:-2]) / 2) * dy)
    Fw[-1][:] = rho * (((u_vel[-3][:-2] + u_vel[-2][:-2]) / 2) * dy + ((u_vel[-2][:-2] + u_vel[-1][:-2]) / 2) * dy/2)
    Fe = rho * ((u_vel[1:-3][1:] + u_vel[2:-2][1:]) / 2) * delta_y
    Fe[0][:] = rho * (((u_vel[0][1:] + u_vel[1][1:]) / 2) * dy/2 + ((u_vel[1][1:] + u_vel[2][1:]) / 2) * dy)
    Fe[-1][:] = rho * (((u_vel[-3][1:] + u_vel[-2][1:]) / 2) * dy + ((u_vel[-2][1:] + u_vel[-1][1:]) / 2) * dy/2)
    ...
    Aw_v = Dw*peclet_function(np.absolute(Pw), "power_law") + np.max(Fw, np.zeros_like(Fw))
    Ae_v = De*peclet_function(np.absolute(Pe), "power_law") + np.max(-Fe, np.zeros_like(Fe))
    As_v = Ds*peclet_function(np.absolute(Ps), "power_law") + np.max(Fs, np.zeros_like(Fs))
    An_v = Dn*peclet_function(np.absolute(Pn), "power_law") + np.max(-Fn, np.zeros_like(Fn))
